...
class train_helpers():
    def __init__(self, folderI=6, timestamps=1, batch_size=1, istrain=True, h=128, w=128, lossMethod='mse', onlyPredict=False, modelN='own'):
        self.dataloader = data_loader_clouds.DatasetHandling(h, w, startT='2020-05-01', endT='2020-11-01',
        ...
        self.dataloader2 = data_loader_clouds.DatasetHandling(h, w, startT='01-01-2015', endT='01-01-2020',
                                                             album='iowa-2015-2020-spa', no_of_timesteps=timestamps,
                                                             batch_size=1, istrain=istrain)
        self.test_dataset_obj = data_loader_clouds.DatasetHandling(h, w, startT='01-01-2015', endT='01-01-2020',
                                                             album='iowa-2015-2020-spa', no_of_timesteps=timestamps,
                                                             batch_size=5, istrain=False)
        self.lossMethod = lossMethod
        self.test_itr = self.dataloader2.load_landsat_images(all_black_clouds=True)
        self.targetH = h
        self.targetW = w
    ...
    def loadplotlosses(self):
        ...
        plt.figure()
        plt.plot(N, losses, 'o-', color='m', label='Training Loss')
        plt.plot(N, lossesT,'*--', color='g', label='Testing Loss')
    ...
    def display_training_images(self, epoch,
        ...
        axarr[0, 0].imshow(cloudy_img_land[:, :, :3])
        axarr[0, 0].set_title(titles[-2], fontdict={'fontsize': 15})

        axarr[0, 1].imshow(target_cloudfree_land[:, :, :3])
        axarr[0, 1].set_title(titles[-1], fontdict={'fontsize': 15})

        axarr[0, 2].imshow(fakeImg[:, :, :3])
        axarr[0, 2].set_title(titles[-1], fontdict={'fontsize': 15})
    ...
    def predictionLosses(self):
        errors = []
        test_itr = self.test_dataset_obj.load_landsat_images(all_black_clouds=True, batch_size=3)
        while True:
            try:
                (cloudy_img_land, target_cloudfree_land, _, _, _) = next(test_itr)
                print("Got image", target_cloudfree_land.shape)
                history = self.generator.evaluate(cloudy_img_land, target_cloudfree_land[:,:,:8], batch_size=3)
        ...
        for epoch in range(0, epochs):
            ...
            if epoch % sample_interval == 0 :
                        self.display_training_images(str(epoch) ,
                                                      inp_sent_img1, inp_prev_landsat1, geo1)
    ...
    def printim(self):
        for i in range(28):
            cloudy_img_land1, target_cloudfree_land1, inp_sent_img1, inp_prev_landsat1, geo1 = self.test_itr.__next__()
...
if __name__ == '__main__':
    ...
    train_helpers=train_helpers(folderI = 100,timestamps = 1, batch_size = 5,  lossMethod='carl', onlyPredict=False,
                                istrain=True, modelN='res')
    # print(train_helpers.generator.summary())

    # train_helpers.printim()
    ...
    # while countI < 5:
    #
    #          cloudy_img_land, target_cloudfree_land, cloud_cov_perc, inp_prev_sent, inp_prev_landsat = train_helpers.trian_itr.__next__()
    ...
    train_helpers.trainOnlyGen(epochs=100, sample_interval=1)
